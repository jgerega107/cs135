...
(define (element? e L)
  (if (eq? L '()) (eq? e '()) (if (eq? e (car L)) (eq? e (car L)) (element? e (cdr L)))))
...
(define (intersection LA LB)
  (if (null? LA)
      '()
      (if(element? (car LA) LB)
         (cons (car (make-set LA)) (intersection (cdr (make-set LA)) (make-set LB)))
         (intersection (cdr LA) LB)
      )
      )
  )
