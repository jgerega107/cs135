...
(define (generate p q e)
  (cons (list (* p q) e) (list(list (* p q) (mul-inv e (lcm (- p 1) (- q 1)))))))
...
(define (mod-exp b e m)
  (if (eq? 0 e)
      1
      (if (eq? 0 (modulo e 2))
          (modulo (expt (mod-exp b (/ e 2) m) 2) m)
...
(define (encrypt plaintext key)
  (map (lambda (c) (mod-exp c (cadr key) (car key))) (map (lambda (c) (char->integer c)) (string->list plaintext))))
...
(define (decrypt ciphertext key)
  (list->string (map (lambda (c) (integer->char c)) (map (lambda (c) (mod-exp c (cadr key) (car key))) ciphertext))))
