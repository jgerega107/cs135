...
;;-------------------------------------------------------------------------------
;; Name: Jacob Gerega
;; Pledge: I pledge my honor that I have abided by the Stevens Honor System.n
;;-------------------------------------------------------------------------------
...
(define (compose RO RI)
  (if (null? RI)
  '()
  (make-set (append (compose-helper RO RI) (compose RO (cdr RI))))
  )
  )

(define (compose-helper RO RI)
  (if (null? RO)
  '()
  (if (eq? (car (reverse (car RI))) (caar RO))
      (cons (cons (caar RI) (cdar RO)) (compose-helper (cdr RO) RI))
      (compose-helper (cdr RO) RI)
      )
  )
)
...
(define (transitive-closure R)
  (transitive-closure-helper R (length R)))

(define (transitive-closure-helper R k)
  (if (eq? k 0)
      '()
      (union (power R k) (transitive-closure-helper R (- k 1)))
      )
)
