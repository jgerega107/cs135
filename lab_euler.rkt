...
(define (get-vertices G)
  (if (null? G)
      '()
      (usort (append (append (list (caar G)) (cdar G)) (get-vertices (cdr G))))
  )
  )
...
(define (degree G v)
  (if (null? G)
      0
      (if (and (eq? (caar G) v) (eq? (cadar G) v))
          (+ 2 (degree (cdr G) v))
          (if (or (eq? (caar G) v) (eq? (cadar G) v))
          (+ 1 (degree (cdr G) v))
          (degree (cdr G) v))
      )
  )
...
(define (all-even? G)
  (all-even-helper (map (lambda (v) (degree G v)) (get-vertices G))))

(define (all-even-helper G)
  (if (null? G)
      #t
      (if (even? (car G))
          (all-even-helper (cdr G))
          #f)
      )
  )
...
(define (adjacent G v)
  (if (null? G)
      '()
      (if (eq? (caar G) v)
          (usort (append (list (cadar G)) (adjacent (cdr G) v)))
          (if (eq? (cadar G) v)
              (usort (append (list (caar G)) (adjacent (cdr G) v)))
              (adjacent (cdr G) v))
          )
      )
  )
...
(define (connected? G)
  (or (null? G) (equal? (get-vertices G) (usort (dfs G '() (caar G))))))
