...
(define (CRT-exists? cong-sys)
  (if (null? cong-sys)
      #t
      (if (CRT-exists-helper (cadar cong-sys) (cdr cong-sys))
          (CRT-exists? (cdr cong-sys))
          #f
      )
  )
  )

(define (CRT-exists-helper a b)
  (if (null? b)
      #t
      (if (eq? (euclid-gcd a (cadar b)) 1)
          (CRT-exists-helper a (cdr b))
          #f
          )
      )
  )
...
(define (m cong-sys)
  (apply * (map (lambda a (cadar a)) cong-sys)))
